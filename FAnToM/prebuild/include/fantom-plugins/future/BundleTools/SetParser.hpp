...
namespace fantom
{
    ...
    public:
        ...
        private:
            ...
            std::vector< int >::const_iterator mListIterator;
        };
         ...
         */
        SetParser( const std::string& description, int vmin = -1, int vmax = -1 )
            : vmin( vmin )
            , vmax( vmax )
        {
            ival = -1;
        ...
        ~SetParser()
        {
            for( std::vector< Set* >::iterator it = mUse.begin(); it != mUse.end(); ++it )
            {
                delete *it;
            }

            for( std::vector< Set* >::iterator it = mDontUse.begin(); it != mDontUse.end(); ++it )
            {
                delete *it;
            }
        }
        ...
        void printError( std::ostream& os )
        {
            if( !err.empty() )
            {
                os << "SetParser encountered an error: \"" << err << "\"." << std::endl;
            }
        }

        /// add a new range to the target set
        void addRange( std::vector< Set* >& target, int a, int b, int step = 1 )
        {
            target.push_back( new Range( a, b, step ) );
        }

        /// add a new list to the target set
        void addList( std::vector< Set* >& target, std::vector< int > values )
        {
            target.push_back( new List( std::move( values ) ) );
        }

        /// read a number from the given string, also resolves the symbols 'u' and 'l'"
        int getNum( const char* where )
        {
            if( where[0] == 'l' )
            {
                return vmin;
        ...
        bool parseRange( std::vector< Set* >& target, const char*& start, const char*& end )
        {
            ...
            int a = getNum( start + 1 );
            int b = getNum( delim + 1 );
        ...
        static const char* usage()
        {
            static const char* const u
                = "SETDEF\n"
                  ...
                  "NUM = non-negative integer value | \"l\" | \"u\"\n";
            return u;
        }

    private:
        int ival; ///< storage for the current value in the iterator is in a valid state
        const int vmin;
        const int vmax;
