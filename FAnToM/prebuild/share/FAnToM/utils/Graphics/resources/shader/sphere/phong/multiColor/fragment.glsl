...
void main()
{
    ...
    #if DEPTH_PEELING
    // Calculate the texel coordinate where the previous depth was stored
    // and compare both depths (normalize the screen point and scale and
    // translate it by 0.5 to match the screen space)
    // Keep the fragment which is further in the back
    ...
    out_color = vec4(
        ( ambient + diffuse ) * frag_color.xyz
