...
void main()
{
    ...
    float q = dot( frag_rayStart, frag_rayStart ) - frag_minScale * frag_minScale;
    ...
    float ambient = 0.15;
    float diffuse = 0.5 * abs( dot( point/frag_minScale, lightDir ) );
    float specular = 0.3 * pow( abs( dot( point/frag_minScale, lightDir ) ), 60.0 );
