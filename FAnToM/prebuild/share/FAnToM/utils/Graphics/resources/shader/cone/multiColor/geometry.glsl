...
in vec3 geom_normal[];
in vec4 vertex_color_i[];
in vec4 vertex_color_j[];
in vec3 pass_radius[];
...
out vec4 fragment_color_i;
out vec4 fragment_color_j;
out vec4 fragment_norm;
out vec3 frag_radius;
...
void main( void )
{
    ...
    fragment_color_i = vertex_color_i[0];
    fragment_color_j = vertex_color_j[0];
    frag_radius = pass_radius[0];
    frag_transform = geom_transform[0];
    
    float radius = max( pass_radius[0].x, max( pass_radius[0].y, pass_radius[0].z ) );
    ...
    for( int cornerId = 0; cornerId < 4; ++cornerId )
    {
        ...
        for( int j = 0; j < 6; ++j )
            gl_ClipDistance[j] = dot( view_inv * vertexPos, clippingPlane[j] );
